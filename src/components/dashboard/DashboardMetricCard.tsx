...
interface DashboardMetricCardProps {
    ...
    viewAllLink?: string; // New prop for navigation
    onViewAllClick?: () => void; // Alternative: callback function
}
            ...
            {!isWallet && (
                <>
                    {viewAllLink ? (
                        <Link
                            to={viewAllLink}
                            className="text-blue-500 dark:text-blue-400 text-xs mt-4 font-medium hover:text-blue-600 dark:hover:text-blue-300 hover:underline transition-colors"
                        >
                            View All →
                        </Link>
                    ) : (
